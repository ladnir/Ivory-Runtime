...
namespace osuCrypto
{
    ...
    //void ShGcRuntime::initInt(sIntBasePtr& data, u64 bitCount)
    //{
    //    if (data) throw std::runtime_error(LOCATION);

    //    data.reset(new ShGcInt(bitCount));
    //}
    ...
    void ShGcRuntime::garblerCircuit()
    {
        ...
        while (mCrtQueue.size())
        {
            ...
            if (item.mCircuit)
            {
                ...
                garble(*item.mCircuit, sharedMem, mTweaks, gates, mZeroAndGlobalOffset);
    ...
    void ShGcRuntime::evaluatorCircuit()
    {
        while (mCrtQueue.size())
        {
            ...
            if (item.mCircuit)
            {
                ...
                if (item.mCircuit->mNonXorGateCount) mChannel->recv((u8*)sharedGates.data(), sharedGates.size() * sizeof(GarbledGate<2>));
